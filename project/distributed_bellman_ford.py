...
path=defaultdict(lambda:defaultdict(lambda:None))
switch_num=4
...
def _get_raw_path (src,dst):
  ...
  for dpid in sws:
    for dpid1 in sws:
      if adjacency[dpid][dpid1] is not None:
        distance[(dpid,dpid1)]=1
    distance[dpid,dpid]=0
  for dpid in sws:
    if adjacency[src][dpid]!=None:
      w=1
      if distance[(dpid,dst)] is not None:
        if distance[(src,dst)] is None:
          distance[(src,dst)]=distance[(dpid,dst)]+w
          previous = dpid
        if distance[(src,dst)] is not None and distance[(dpid,dst)]+w<=distance[(src,dst)]:
          distance[(src,dst)]=distance[(dpid,dst)]+w
          previous = dpid
  r=[]
  path[src,dst]=[]
  path[(src,dst)]=path[(src,dst)]+[dpid]
  path[(src,dst)]=path[src,dst]+path[(dpid,dst)]
  p=src
  r.append(p)
  q=previous
  r.append(q)
