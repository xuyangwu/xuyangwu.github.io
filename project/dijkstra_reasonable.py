...
def _get_raw_path (src,dst):
  ...
  for m in range(len(sws)-1):
    ...
    for q in sws:
      if visited[q]==0:
        if weight(min_index,q) + min_distance < distance[q]:
          distance[q] = weight(min_index,q) + min_distance
          previous[q] = min_index
    if visited[dst]==1:
      break
  r=[]
  p=dst
...
mininet@mininet-vm:~/pox/pox/forwarding$ vim dijkstraversion2.py                                                                                                        mininet@mininet-vm:~/pox/pox/forwarding$ vim dijkstraversion2.py
